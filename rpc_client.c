...
void rpc_call(struct ib_resources_t *ib_resources,
    ...
    wr.num_sge = 0;
    wr.opcode = IBV_WR_SEND_WITH_IMM;/* TODO: fill in this. refer to the man page or to the references mentioned in the homework PDF */
    ...
    if (result) {
        ...
        *result_size = wc.imm_data;
        ...
        struct ibv_mr *mr = ibv_reg_mr(ib_resources->pd, result, IBV_ACCESS_LOCAL_WRITE);
        if (!mr) {
            printf("ERROR: ibv_reg_mr failed\n");
            exit(1);
        }
        ...
        sg.addr = (uintptr_t)mr->addr;
        sg.length = *result_size;//result_size
        sg.lkey = mr->lkey;
